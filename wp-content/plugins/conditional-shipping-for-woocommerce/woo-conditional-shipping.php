...
class Woo_Conditional_Shipping {
	...
	public function admin_enqueue_scripts() {
		...
		wp_enqueue_script( 'woo_conditional_shipping_js', plugin_dir_url( __FILE__ ) . '/admin/js/woo-conditional-shipping.js', array( 'jquery' ) );
	...
	public function process_options( $instance_settings, $method ) {
		...
		foreach ( $post_data as $key => $value ) {
			if (strpos($key, '_wcs_type_') !== false) {
				$settings_key = $settings_prefix . '_wcs_type_';
				$index = str_replace( $settings_key, '', $key );
				$type = $post_data[$settings_prefix . '_wcs_type_' . $index];
				$value = $post_data[$settings_prefix . '_wcs_value_' . $index];
				$shipping_classes = $post_data[$settings_prefix . '_wcs_value_shipping_class_ids_' . $index];
				$categories = $post_data[$settings_prefix . '_wcs_value_category_ids_' . $index];
				$products = $post_data[$settings_prefix . '_wcs_value_product_ids_' . $index];

				$instance_settings['wcs_conditions'][] = array(
					'type' => $type,
					'value' => $value,
					'shipping_class_ids' => $shipping_classes,
	...
	private function generate_table_html( $method ) {
		...
		return '<table class="form-table wcs-conditions-table" data-selected-products="' . htmlspecialchars( json_encode( $products ), ENT_QUOTES, 'UTF-8' ) . '" data-conditions="' . htmlspecialchars( json_encode( $conditions ), ENT_QUOTES, 'UTF-8' ) . '"><tbody>' . $this->generate_rows_html( $method ) . '</tbody>' . $this->generate_tfoot_html() . '</table>';
	}
		...
		<script type="text/html" id="tmpl-wcs_row_template">
			<tr valign="top" class="condition_row">
				...
				<th scope="row" class="titledesc">
					<fieldset>
						<select name="<?php echo $method->get_field_key('wcs_type'); ?>_{{data.index}}" class="wcs_condition_type_select">
				...
				<td class="forminp">
					<fieldset class="wcs_condition_value_inputs">
						<input class="input-text value_input regular-input wcs_text_value_input" type="text" name="<?php echo $method->get_field_key('wcs_value'); ?>_{{data.index}}" value="{{data.value}}" />

						<div class="value_input wcs_product_value_input">
							<select class="wcs_product_value_ajax_input" name="<?php echo $method->get_field_key( 'wcs_value_product_ids' ); ?>_{{data.index}}[]" class="select" multiple>
	...
	public function exclude_shipping_methods( $rates, $package ) {
		foreach( $rates as $key => $rate ) {
			...
			if ( isset( $method->instance_settings['wcs_conditions'] ) && is_array( $method->instance_settings['wcs_conditions'] ) ) {
				foreach ( $method->instance_settings['wcs_conditions'] as $index => $condition ) {
	...
	private function _filter_product_exclude( $condition, $package ) {
		if ( isset( $condition['product_ids'] ) && ! empty( $condition['product_ids'] ) ) {
			$exclude_product_ids = $condition['product_ids'];

			foreach ( $package['contents'] as $key => $item ) {
				$product = $item['data'];
				$product_id = $product->get_id();

				if ( in_array( $product_id, $exclude_product_ids ) ) {
					return TRUE;
				}
			}
		}
	...
	private function _filter_product_include( $condition, $package ) {
		if ( isset( $condition['product_ids'] ) && ! empty( $condition['product_ids'] ) ) {
			$include_product_ids = $condition['product_ids'];

			foreach ( $package['contents'] as $key => $item ) {
				$product = $item['data'];
				$product_id = $product->get_id();

				if ( in_array( $product_id, $include_product_ids ) ) {
					return FALSE;
				}
			}
	...
	private function _filter_product_exclusive( $condition, $package ) {
		if ( isset( $condition['product_ids'] ) && ! empty( $condition['product_ids'] ) ) {
			$exclusive_product_ids = $condition['product_ids'];

			foreach ( $package['contents'] as $key => $item ) {
				$product = $item['data'];
				$product_id = $product->get_id();

				if ( ! in_array( $product_id, $exclusive_product_ids ) ) {
					return TRUE;
				}
			}
		}
