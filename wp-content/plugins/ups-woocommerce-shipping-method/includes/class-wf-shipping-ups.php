...
class WF_Shipping_UPS extends WC_Shipping_Method {
    ...
    private function per_item_shipping( $package, $params=array() ) {
	    ...
	    $requests = array();
        //$all_package_weight[] = $length_of_all_item[] = $width_of_all_item[] = $height_of_all_item[] = array();
		$ctr=0;
    	...
    	foreach ( $package['contents'] as $item_id => $values ) {
			...
			$weight = $weight*$cart_item_qty;
			$all_package_weight[] = $weight;
		
			for ( $i=0; $i < $cart_item_qty ; $i++){

				$length_of_all_item[] = $dimensions[2];
				$width_of_all_item[] = $dimensions[1];
				$height_of_all_item[] = $dimensions[0];
			}

            if( count( $package['contents'] ) ==$ctr ){

            $weight = array_Sum( $all_package_weight );  
            $request  = '<Package>' . "\n";
			...
			$request .= '	<Description>Rate</Description>' . "\n";
				...
				$request .= '		</UnitOfMeasurement>' . "\n";
				$request .= '		<Length>' . max( $length_of_all_item ) . '</Length>' . "\n";
				$request .= '		<Width>' . max( $width_of_all_item ) . '</Width>' . "\n";
				$request .= '		<Height>' . max( $height_of_all_item ) . '</Height>' . "\n";
				$request .= '	</Dimensions>' . "\n";
